...
fn handle_request_routes_different_paths_without_tcp_socket() {
    ...
    let home_response = response_text(router.handle_request(&home_request, &mut state));
    let health_response = response_text(router.handle_request(&health_request, &mut state));
...
fn handle_request_uses_method_when_matching_routes() {
    ...
    let get_response = response_text(router.handle_request(&get_request, &mut state));
    let post_response = response_text(router.handle_request(&post_request, &mut state));
