...
- `handle_client` still performs one read into a fixed buffer. A valid request can arrive partially and be treated as malformed. This is intentional until Phase 06A adds minimal accumulation; Phase 13 will later harden it with limits, timeouts, and explicit connection behavior.
